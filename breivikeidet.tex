...
For this work WRF model version 3.8 was used. Below, different parameters of the WRF model that were used in this work are explained shortly, but the reader is encouraged to see appendix ... for a complete list of parameters that were given to the WRF model in the form of a namelist.input file.
...
WRF model is based on a third order Rungekuta integration. Therefore, time step is one of the core parameters of this model and special attention has to be paid to selecting the right time step. A very small time step can be computationally very expensive and a very big time step can cause an early divergence which in turn causes the simulation to crash. In order to find the appropriate value for time step the given approach in [ref] was followed. They introduced the following criteria (eq. 3.1) based on which the value of $dt$ should be confined to a certain interval.
\vspace{0.25cm}
\begin{equation}
t = ...
\end{equation}
